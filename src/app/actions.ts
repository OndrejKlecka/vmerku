...
export async function searchCandidates(
  ...
  const selected = await db
    ...
    .where(
      and(
        ...
        inArray(stores.id, storeIds.length ? storeIds : [-1]),
      ),
    )
    .all();
  ...
  for (const store of selected) {
    ...
    const ranked = rankCandidates(trimmed, pool, (i) => i.rawName, {
      threshold: 0.4,
      limit: 3,
    });
    if (ranked.length === 0) {
...
export async function saveSettings(formData: FormData): Promise<void> {
  ...
  const notifyMode =
    formData.get("notifyMode") === "daily-digest" ? "daily-digest" : "instant";
...
/** Společné čtení formuláře obchodu pro přidání i úpravu. */
function storeFields(formData: FormData) {
  const name = String(formData.get("name") ?? "").trim();
  const sourceUrl = String(formData.get("sourceUrl") ?? "").trim();
  const kind =
    formData.get("kind") === "daily-scrape" ? "daily-scrape" : "weekly-leaflet";
  ...
  return {
    ...
    sourceUrl,
    leafletDay:
      kind === "weekly-leaflet" && leafletDayRaw ? Number(leafletDayRaw) : null,
  } as const;
}

export async function addStore(formData: FormData): Promise<void> {
  const fields = storeFields(formData);
...
/** Úprava obchodu; historie cen i barva v grafu zůstávají. */
